    ...
    this.eventChannel.subscribeAlways<FriendMessageEvent>(priority= EventPriority.LOWEST) { event ->
        when (event.message.content.trim()) {
            "绑定" -> {
                ...
                val result = Mongo.bind(user._id, subject.id)
                if (result.modifiedCount == 1L) {
                    subject.sendMessage("绑定成功\n\n" +
                            "本次绑定服务结束")
                } else {
                    subject.sendMessage("绑定失败，请联系开发者qq：${System.getenv("SP_ADMIN")}，code: 101, debug: ${result.matchedCount}|${result.modifiedCount}\n\n" +
                            "本次绑定服务结束")
                }
            }
            "解绑" -> {
                val user = Mongo.getUser(subject.id) ?: run {
                ...
                if (result.modifiedCount == 1L) {
                    ...
                    subject.sendMessage("解绑失败，请联系开发者qq：${System.getenv("SP_ADMIN")}，code: 101, debug: ${result.matchedCount}|${result.modifiedCount}\n\n" +
                            "本次解绑服务结束")
                }
            }
            "查余额" -> {
                ...
                val area = if (user.position.custom.state && user.position.custom.area != null) user.position.custom.area else user.position.area ?: run {
                    subject.sendMessage("无法获知你的校区，请联系开发者qq：${System.getenv("SP_ADMIN")}\n\n" +
                            "本次查电费服务结束")
                    return@subscribeAlways
                }
                val building = if (user.position.custom.state && user.position.custom.building != null) user.position.custom.building else user.position.building ?: run {
                    subject.sendMessage("无法获知你的宿舍楼，请联系开发者qq：${System.getenv("SP_ADMIN")}\n\n" +
                            "本次查电费服务结束")
                    return@subscribeAlways
                }
                val room = if (user.position.custom.state && user.position.custom.room != null) user.position.custom.room else user.position.room ?: run {
                    subject.sendMessage("无法获知你的寝室，请联系开发者qq：${System.getenv("SP_ADMIN")}\n\n" +
                            "本次查电费服务结束")
                    return@subscribeAlways
                }
